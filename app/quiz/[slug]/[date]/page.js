...
export function generateMetadata({ params }) {
  ...
  return {
    ...
    // 정답 0건인 날짜 페이지는 색인하지 않는다.
    //
    // 2026-08-10 실측: 날짜×퀴즈 페이지 575개 중 196개(34%)가 정답 0건이다.
    // 소스가 없었거나 그날 퀴즈 자체가 없던 날들이다. 이런 빈 페이지가 색인되면
    // 검색해 들어온 사람이 빈 화면을 보고 바로 나가고, 검색엔진은 우리 도메인을
    // '내용 없는 사이트'로 학습한다. follow는 남겨 내부 링크는 계속 타게 한다.
    // 오늘 페이지·월간 페이지는 대상이 아니다 — 고정 주소이고 최근 정답으로
    // 화면이 채워지므로 빈 페이지가 아니다.
