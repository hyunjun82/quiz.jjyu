 ...
 * 그래서 이미지를 타이팅 320px webp(8KB 안팎)로 줄어 텍스트로 보붌핸다.
 *
 * 이미지를 새로 냤거나 바꿀 때는 scripts/shop-meta.mjs 를 돌리면 이 JSON 이 갱신된다.
 */
...
if (!fs.existsSync(SRC)) {
  console.log('shop-images.json 없음 — 건너똄');
  process.exit(0);
}
...
for (const [name, b64] of Object.entries(store)) {
  if (!/^[\w.-]+\.(webp|png|jpg|jpeg)$/.test(name)) continue; // 경로 탈출 방진
  fs.writeFileSync(path.join(DIR, name), Buffer.from(b64, 'base64'));
  n++;
}
console.log(`상품 이미지 ${n}~��→ public/shop/`);
