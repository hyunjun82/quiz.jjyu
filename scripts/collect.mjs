...
function isSaneAnswer(a) {
  ...
  // 실재한다 — 시각만 달랑 있으면 정답으로 보고, 시각 앞뒤에 다른 말이 붙어 있을 때만 잡담으로 본다.
  if (/\d{1,2}시 \d{1,2}분/.test(a) && !/^(오전|오후)?\s*\d{1,2}시\s*\d{1,2}분$/.test(a)) return false;
  // 7/28 실측: 카카오페이에 정답 "- 99"가 발행됐다. 커뮤니티 글 본문 조각이
  // 정답 칸에 그대로 들어온 것. 정답은 문장부호로 시작하지 않는다.
  if (/^[-–—·.,?!/|]/.test(a)) return false;
  if (!/[가-힣A-Za-z0-9]/.test(a)) return false;
  // 8/28 실측: 퀴즈벨 토스 칸에 커뮤니티가 여러 퀴즈를 한 줄로 뭉친 잡담이 들어온다
  // ("아이폰 웰스 브랜드데이정답 - 52, 카포드" / 8/20 "…퀴즈정답 - 치킨").
  // 중간에 '정답 -'/'정답:'이 끼면 잡담이다. "정답 : 수원"처럼 앞에 붙는 건 정상이라 허용.
  // 전 기간(7/1~8/28) 발행 정답 전수 대조: 이 규칙에 걸리는 진짜 정답 0건.
  if (!/^정답/.test(a) && /정답\s*[-–—:]/.test(a)) return false;
  return true;
}
 ...
 */
// 9/4 실측: 토스행운퀴즈(랜덤 출제형) 안내문 "사용자마다 문제가 랜덤으로 다르게
// 출제됩니다."가 진짜 문제로 4회 이상 반복 저장됐다(17:55/19:06/19:55/20:56 커밋에서
// 매번 손으로 지움). 기존 /문제는 랜덤입니다/(TIP_REVIEWED, 제목 매칭)는 이 안내문
// 자체가 "질문 지문"으로 들어오는 경로를 못 잡는다 — 여기서 지문 내용으로 직접 막는다.
...
function isSaneQuestion(q) {
  ...
  if (!s) return false;
  if (GUIDANCE_PHRASES.some((re) => re.test(s))) return false;
  ...
  // 8/18 실측: 퀴즈벨 토스 표에 "가나브이기리니기리니 = 31" 이 섞여 들어와
  // 그대로 발행됐다. 사람이 키보드를 두드린 행이다. 위 규칙들은 숫자·URL만 보므로
  // 한글 장난 글은 전부 통과한다. 두 가지를 더 본다.
  //
  // ⚠️ 진짜 문제를 버리면 훨씬 큰 손해라, 아주 좁게 잡는다.
  //    아래 두 규칙은 지금까지 수집한 전체 지문에 대해 오탐 0을 확인하고 넣었다.

  // (1) 한글 자모만: "ㅋㅋㅋㅋ", "ㅁㄴㅇㄹ". 실제 문제에는 자모만 쓰인 지문이 없다.
  //     (초성 힌트는 "(초성: ㅇㅋㅅㅇ)" 처럼 괄호 안에 들어오므로 여기 안 걸린다.)
  if (/^[ㄱ-ㅎㅏ-ㅣ\s]+$/.test(s)) return false;

  // (2) 같은 덩어리가 바로 이어서 반복되고 그게 문자열의 절반 이상인 경우.
  //     "가나브이기리니기리니" → "기리니"가 두 번, 전체 10자 중 6자.
  //     띄어쓰기가 있는 문장은 제외한다 — 진짜 문제는 거의 다 띄어쓰기가 있다.
  if (!/\s/.test(s) && s.length <= 24) {
    const rep = s.match(/(.{3,6})\1/);
    if (rep && rep[0].length / s.length >= 0.5) return false;
  }
...
const TIP_TITLE_MAP = [
  ...
  // 2026-08-18 추가. TIP_UNMAPPED 보고로 발견된 걸 사장님 지시로 편입한다.
  // 팁is팁 말고 다루는 소스가 없어서 backup 이 아니라 '항상 사용'이다.
  //   제목: "네이버페이] 랜덤 포인트 퀴즈 [2026년 08월 18일]"
  //   본문: "네이버페이] 랜덤 포인트 퀴즈 / 정답 : 정보태그" — 지문 없이 정답만 온다.
  // 발행 빈도는 매일이 아니다. 게시 이력 실측(7/5~8/18): 7월 9회, 8월 6회.
  { slug: 'naverpay', re: /^네이버페이\]/ },
  // ── 빈 곳 메우기 전용(backup) ──
...
function parseTipArticle(html, slug, title) {
  ...
  //
  // ⚠️ 2026-08-18 사고: 예전엔 오전·오후가 '둘 다' 있을 때만 쪼갰다. 그래서 오전
  // 정답만 먼저 올라온 아침에 `정답 : 오전 - 연봉상승 물약` 이 통째로 한 건이 되고,
  // 오후 정답이 붙은 뒤 다시 두 건이 들어와 같은 정답이 세 줄로 발행됐다.
  // 한쪽만 있어도 쪼갠다 — 그래야 나중에 들어오는 것과 지문이 같아져 중복으로 걸린다.
...
// monimo(모니스쿨): 한 문제를 일주일 단위로 건다. 2026-09-17 30일치 실측 —
//   8/19~8/23 '아이스크림'(5일) · 8/23~9/1 'X'(9일) · 9/1~9/8 '명동'(8일)
//   9/8~9/15 'X'(7일) · 9/15~ '분홍빛을 띠고 매끄럽다'
// 며칠씩 같은 것이 정상이므로 '어제와 동일' 차단을 걸면 안 된다.
