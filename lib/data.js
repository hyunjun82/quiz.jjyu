...
export function getShopPicks() {
  ...
  try {
    ...
    // 이미지 경로는 링크 해시로 계산한다 — shop.json 에 손으로 적지 않는다.
    // (적게 했더니 새 상품 넣을 때 빼먹어서 사진이 통째로 안 나왔다. 2026-08-20)
    // 파일은 빌드 직전 scripts/shop-images.mjs 가 같은 규칙으로 받아 놓는다.
    // 못 받았으면(토스 쪽 실패 등) 깨진 사진 대신 사진 없는 카드로 내보낸다.
    items = items.map((it) => {
      const key = crypto.createHash('sha1').update(it.link).digest('hex').slice(0, 10);
      const rel = `/shop/${key}.webp`;
      const f = path.join(process.cwd(), 'public', 'shop', `${key}.webp`);
      return { ...it, image: fs.existsSync(f) ? rel : '' };
    });
    headline = j.headline ?? '';
  } catch {
    items = [];
...
export function formatTime(iso) {
  if (!iso) return '';
  // ICU 에 기대지 않는다. 2026-09-14 실측: 빌드를 GitHub Actions(Node 22)로 옮기자
  // toLocaleTimeString('ko-KR') 이 "오후 03:46" 대신 "PM 03:46" 을 내놓아 메인 문구가 바뀌고
  // 감사 스크립트(quiz-audit)의 "업데이트 시각" 파서가 깨졌다. KST 로 직접 계산해 항상 같은 모양을 낸다.
